                ...
                <div class="card">
                    <div class="card-body">
                        <section class="d-flex justify-content-between align-items-center">
                            ...
                            <article>
                                <a href=" {{ url('vehicular/administracion/paquetes') }}">
...
@section('scripts')
    ...
    <script>
        ...
        function agregarProducto(id) {
            ...
            productoEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, cantidad, IdUnidadMedida);
            return 0;
        }

        function productoEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, cantidad, IdUnidadMedida) {
            // Nuevo codigo
            if (IdUnidadMedida == 1) {
                step = '';
            } else {
            ...
            var fila = '<tr class="text-center" id="row' + iden + '"><td id="pro' + iden + '">' + 'PRO-' + id +
                ...
                '<td>' + codigoBarra + '</td>' +
                // Nuevo codigo
                '<td><input id="cant' + iden + '" name="Cantidad[]" step="' + step +
        ...
        function agregarServicio(id) {
            if (arrayIds.includes(id) == true) {
                ...
                servicioEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, cantidad);

            }
        }

        function servicioEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, cantidad) {
            $('#tablaAgregado tr:last').after('<tr class="text-center"  id="row' + iden + '"><td  id="pro' + iden + '">' +
                ...
                '<td>' + codigoBarra + '</td>' +
                // Nuevo codigo
                '<td><input id="cant' + iden + '" name="Cantidad[]" step="any" type="number" min="1" value="' +
                cantidad + '" style="width:60px">' +
                // Fin
                '<td><button id="btn' + iden + '" onclick="eliminarFila(' + iden + ',' + id +
                ')" class="btn btn-primary"><i class="list-icon material-icons fs-16">clear</i></button>' +
                '</td>' +
    ...
    <script>
        $(function() {
            var itemsGrupo = @json($itemsGrupo);
            for (var i = 0; i < itemsGrupo.length; i++) {
                datosProductos = new Array();
                if (itemsGrupo[i]["CantidadArticulo"] == null) {
                    var cantidadArticulo = '1.00';
                } else {
                    var cantidadArticulo = itemsGrupo[i]["CantidadArticulo"];
                }
                if (parseInt(itemsGrupo[i]["idTipoArticulo"]) == 1) {
                    productoEnTabla(itemsGrupo[i]["IdArticulo"], itemsGrupo[i]["Descripcion"], itemsGrupo[i][
                        ...
                        itemsGrupo[i]["codigoBarra"], cantidadArticulo, itemsGrupo[i][
                            "IdUnidadMedida"
                        ])
                } else {
