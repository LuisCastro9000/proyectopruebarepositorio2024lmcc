                ...
                <div class="card">
                    <div class="card-body">
                        <section class="d-flex justify-content-between align-items-center">
                            <h6>Editar Paquete Promocional</h6>
                            <article>
                            ...
                            <article class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="nombre" type="text" name="nombre"
                                        autocomplete="off" value="{{ $nombrePaquete }}">
                                    <input hidden id="idPaquete" class="form-control" type="text"
                                        value="{{ $idPaquete }}" readonly>
...
@section('scripts')
    ...
    <script>
        ...
        function productoEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, costo, cantidad,
            ...
            var fila = '<tr class="text-center" id="row' + iden + '"><td id="pro' + iden + '">' + 'PRO-' + id +
                ...
                '</td><td><input id="importe' + iden + '" name="Importe[]" step="any" readonly type="number" value="' +
        ...
        function servicioEnTabla(id, descripcion, marca, categoria, precio, stock, codigoBarra, costo, cantidad,
            ...
            $('#tablaAgregado tr:last').after('<tr class="text-center"  id="row' + iden + '"><td  id="pro' + iden + '">' +
                ...
                '<td>' + costo + '</td>' +
                '<td hidden><input id="costoArticulo' + iden + '" value="' + costo + '">' + '</td>' +
                '<td><input id="cant' + iden + '" name="Cantidad[]" step="any" onchange="calcular(this, ' + iden +
                ');" type="number" min="1" value="' + cantidad + '" style="width:60px">' +
                '</td><td><input id="importe' + iden + '" name="Importe[]" step="any" readonly type="number" value="' +
                redondeo(importeFinal) + '" style="width:100px">' +
                '</td><td hidden><input id="impCosto' + iden +
        ...
        function calcular(idRow, iden) {
            ...
            $('#importe' + iden).val(redondeo(importe));
            ...
            console.log('calcular costo' + totalCosto);
            $('#totalPaquete').val(redondeo(total));
            $('#costoPaquete').val(redondeo(totalCosto));
        }

        function eliminarFila(id, i) {
            var importeArticulo = $('#importe' + id).val();
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $('#row' + id).remove();
                        $('#' + id).remove();
                        total -= parseFloat(importeArticulo)
                        totalCosto -= parseFloat(importeCosto);
                        $('#totalPaquete').val(redondeo(total));
                        $('#costoPaquete').val(redondeo(totalCosto));
        ...
        function enviar() {
            ...
            swal({
                    ...
                    text: "Una vez actualizado el Paquete, no podrá recuperar el Paquete anterior!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                .then((willActualizar) => {
                    if (willActualizar) {
                        ...
                        var idsItems = arrayIds;
                        $.ajax({
                            type: 'post',
                            url: '../actualizar-paquete-promocional',
                            data: {
                                "_token": "{{ csrf_token() }}",
                            ...
                            success: function(data) {
                                ...
                                if (data[0] == 'alert1') {
                                    ...
                                    if (data[0] == 'alert2') {
                                        swal("Falta Ingresar items al Paquete",
                                            "Por favor ingrese los Items");
                                    } else {
                                        if (data[0] == 'alert3') {
                                            swal("El paquete ya existe", "Por favor agregue otro nombre");
                                        } else {
                                            if (data[0] == 'error') {
                                                swal("Surgio un error, comunicarse con soporte");
                                            } else {
                                                swal({
                                                        title: "El Paquete se actualizo Correctamente!",
                                                        icon: "success",
                                                        button: "Entendido",
                                                    })
                                                    .then((Entendido) => {
    ...
    <script>
        $(function() {
            var itemsPaquete = @json($itemsPaquete);
            console.log(itemsPaquete);
            for (var i = 0; i < itemsPaquete.length; i++) {
                datosProductos = new Array();
                if (parseInt(itemsPaquete[i]["idTipoArticulo"]) == 1) {
                    productoEnTabla(itemsPaquete[i]["IdArticulo"], itemsPaquete[i]["Descripcion"], itemsPaquete[i][
                            "nombreMarca"
                        ], itemsPaquete[i]["nombreCategoria"], itemsPaquete[i]["Precio"], itemsPaquete[i][
                            "Stock"
                        ],
                        itemsPaquete[i]["codigoBarra"], itemsPaquete[i]["Costo"], itemsPaquete[i]["Cantidad"],
                        itemsPaquete[i]["IdUnidadMedida"])
                } else {
                    servicioEnTabla(itemsPaquete[i]["IdArticulo"], itemsPaquete[i]["Descripcion"], itemsPaquete[i][
                            "nombreMarca"
                        ], itemsPaquete[i]["nombreCategoria"], itemsPaquete[i]["Precio"], itemsPaquete[i][
                            "Stock"
                        ], itemsPaquete[i]["codigoBarra"], itemsPaquete[i]["Costo"], itemsPaquete[i][
                            "Cantidad"
                        ],
                        itemsPaquete[i]["IdUnidadMedida"])
                }
