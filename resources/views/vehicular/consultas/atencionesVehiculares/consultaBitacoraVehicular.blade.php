...
<body class="sidebar-horizontal">
                ...
                <div class="container mt-2">
                    <div class="row page-title clearfix mt-4">
                        ...
                        <div class="page-title-right">
                            @if ($bitacoraSelect->IdCotizacion != null)
                                <section class="d-fex flex-column">
                                    <article>
                                        <a target="_blank" class="fs-18"
                                    ...
                                    <article class="text-center">
                                        <span class="badge badge-warning fs-16">cotización</span>
                                    </article>
                                </section>
                            @endif
                        </div>
                    ...
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            @foreach ($atencionesVehiculares as $bitacoraSelect)
                                <a class="nav-item nav-link {{ $bitacoraSelect->Documento }} font-weight-bolder"
                                    id="nav-home-tab" data-toggle="tab" href="#{{ $bitacoraSelect->Documento }}"
                                    role="tab" aria-controls="nav-home"
                                    aria-selected="true">{{ $bitacoraSelect->Documento }}</a>
                            @endforeach
                            <a class="nav-item nav-link ultimoTab" id="nav-home-tab" data-toggle="tab" href="#nav-casa"
                                role="tab" aria-controls="nav-home" aria-selected="true"></a>
                        </div>
                    </nav>
                    <div class="tab-content" id="nav-tabContent">
                        @foreach ($atencionesVehiculares as $bitacoraSelect)
                            <div class="tab-pane fade" id="{{ $bitacoraSelect->Documento }}" role="tabpanel"
                                ...
                                <div class="ecommerce-invoice">
                                                ...
                                                <article>
                                                    <a class="p-1"
                                                        ...
                                                        target="_blank"><button
                                                            class="btn btn-block btn-info ripple"><i
                                                                class="list-icon material-icons fs-20">picture_as_pdf</i>Descargar</button></a>
                                                </article>
                                            </section>
                                        </div>
                                    ...
                                    <div class="d-sm-flex">
                                        <div class="col-md-6 mt-2">
                                            ...
                                            {{ $bitacoraSelect->Direccion }}
                                        </div>
                                        <div class="col-md-6 text-right mt-2 d-none d-sm-block">
                                            <h6>Detalles:</h6>
                                            <strong>FECHA:</strong> <span
                                                class="text-muted">{{ $bitacoraSelect->FechaAtencion }}</span>
                                            <br><strong>HORA:</strong> <span
                                                class="text-muted">{{ $bitacoraSelect->HoraAtencion }}</span>
                                            ...
                                            <br><strong>TIPO MONEDA:</strong> <span class="text-muted">
                                                @if ($bitacoraSelect->IdTipoMoneda == 1)
                                                    Soles
                                                @else
                                                    Dólares
                                                @endif
                                                ...
                                                <div class="row d-sm-flex mt-3 mb-2">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <div><strong>VENC. REV. TÉCNICA: </strong>
                                                            {{ $bitacoraSelect->FechaRevTecnica }}
                                                        </div>
                                                    </div>
                                    ...
                                    <table id="table" class="table table-bordered table-responsive-lg"
                                        ...
                                        <tbody>
                                            @foreach ($bitacoraSelect->Productos as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->Cantidad }}</td>
                                                    <td>{{ number_format($item->Importe, 2, '.', ',') }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    ...
                                    <div class="row mt-4">
                                        <div class="col-md-8">
                                            <div class="form-group">
                                                ...
                                                <textarea id="observacion" class="form-control" rows="4">{{ $bitacoraSelect->Observacion }}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-4 invoice-sum">
                                            <ul class="list-unstyled">
                                                ...
                                                <li><strong>TOTAL :
                                                        {{ $bitacoraSelect->Total }}</strong>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="form-actions btn-list mt-3">
                                        {{-- <a href="javascript: history.go(-1)"><button onclick="history.go(-1)" class="btn btn-info"
    ...
    <script>
        $(function() {
            let $documento = @json($atencionesVehiculares);
            let $primerDocumento = $documento[0].Documento;
            let $tab1 = document.querySelector('.' + $primerDocumento);
            $tab1.click();
        })
    </script>
</body>
