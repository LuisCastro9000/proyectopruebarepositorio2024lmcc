...
<body>
    <div class="container">
        <table width="100%" class="tabla1">
                    ...
                    <table width="100%">
                        <tr>
                            <td align="center" style="border: 1px solid #000;margin:10px;">
                                <div>
                                    <span class="h2">GUÍA DE REMISIÓN </span><br />
                                    <span class="h2">ELECTRÓNICA - REMITENTE</span>
                                </div>
                                <div style="margin-top: 5px;">
                                    <span class="h2">RUC: {{ $empresa->Ruc }}</span>
                                </div>
                                <div style="margin-bottom: 15px;">
                                    <span class="h2">{{ $guiaSelect->Serie }}-{{ $numeroCeroIzq }}</span>
                                </div>
                            </td>
                        </tr>
                    </table>
            ...
            <tr>
                ...
                @if ($guiaSelect->Principal != 1)
                    <td align="left">SUCURSAL: {{ $guiaSelect->Sucursal }}</span></td>
                @endif
            </tr>
            <tr>
                <td>{{ $empresa->Direccion }} - {{ $empresa->Ciudad }}</td>
                @if ($guiaSelect->Principal != 1)
                    <td align="left">DIRECCION: {{ $guiaSelect->Local }} {{ $guiaSelect->Ciudad }}</td>
                @endif
            </tr>
            <tr>
                <td>TELÉFONO: {{ $empresa->Telefono }}</td>
                @if ($guiaSelect->Principal != 1)
                    <td align="left">TELÉFONO: {{ $guiaSelect->TelfSucursal }}</td>
                @endif
            </tr>
        </table>
        ...
        <table width="100%" class="tabla2">
            ...
            <tr>
                <td width="20%">MOTIVO DEL TRANSLADO:</td>
        ...
        <table width="100%" class="tabla2">
            ...
            <tr>
                <td width="40%">DIRECCION DEL PUNTO DE PARTIDA:</td>
                <td width="40%">{{ $guiaSelect->Origen }}</td>
                <td width="10%">&nbsp;</td>
                <td width="10%">&nbsp;</td>
            </tr>
            <tr>
                <td width="40%">DIRECCION DEL PUNTO DE LLEGADA:</td>
