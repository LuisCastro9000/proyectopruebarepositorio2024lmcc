  @extends('layouts.app')
  @section('title', 'Reporte de clientes Top')
  @section('content')
      <div class="container">
          @if (session('status'))
          ...
          @endif
          {!! Form::open(['url' => '/reportes/gerenciales/clientes-top', 'method' => 'POST', 'files' => true]) !!}
          {{ csrf_field() }}
              ...
              <div class="col-md-2 col-6 mt-4 order-md-3">
                  <div class="form-group text-center text-md-left">
                      ...
                      <a class="" target="_blank"
                          href='{{ url("reportes/gerenciales/clientesTop/excel-clientesTop/$IdTipoPago/$fecha/$ini/$fin") }}'>
                      ...
                      <div class="widget-bg">
                          ...
                          <div class="widget-body clearfix">
                              ...
                              <table id="table" class="table table-responsive-sm" style="width:100%">
                                  ...
                                  <tbody>
                                      @foreach ($reporteClientesTop as $reportClienteTop)
                                          <tr>
                                              <td>{{ $reportClienteTop->Nombre }}</td>
                                              <td>{{ $reportClienteTop->Documento }} :
                                                  {{ $reportClienteTop->NumeroDocumento }}</td>
                                              <td>{{ $reportClienteTop->Telefono }}</td>
                                              <td>{{ $reportClienteTop->Email }}</td>
                                              @if ($reportClienteTop->IdTipoPago == 1)
                                                  <td>Contado</td>
                                              @else
                                                  <td>Crédito</td>
                                              @endif
                                              <td>{{ $reportClienteTop->Cantidad }}</td>
                                          </tr>
                                      @endforeach
                                  </tbody>
                              </table>
          ...
          <div class="modal-dialog">
              <div class="modal-content">
                  <div class="modal-header">
                      <h6 class="text-success">Reportes Gerencial - Clientes Top</h6>
                  </div>
                  <div class="modal-body form-material">
                      <div>
                          <label class="fs-14 negrita">Reporte del Mes</label>
                          <p class="fs-15negrita">Se mostraran solo los clientes top de este mes....... Si desea ver
                              clientes top anteriores utilize los filtros</p>
                      </div>
                  </div>
