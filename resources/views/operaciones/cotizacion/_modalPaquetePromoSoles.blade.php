<div class="modal fade bs-modal-lg-paquetesPromocionales-soles" role="dialog" aria-labelledby="myLargeModalLabel"
            ...
            <div class="modal-body">
                <div class="tabs tabs-bordered">
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab-paquetesPromocionales-soles">
                            <div class="clearfix">
                                <div class="form-group">
                                    <i class="feather feather-search pos-absolute pos-right vertical-center pr-4"></i>
                                    <input type="search" id="inputBuscarPaquetePromocionalSoles" name="textoBuscar"
                                        placeholder="Buscar grupo..."
                                        class="form-control form-control-rounded heading-font-family fs-16 pr-5 pl-3">
                                </div>

                                <!-- Products List -->
                                <div id="cardPaquetesPromocionalesSoles" class="ecommerce-products list-unstyled row">
                                    @foreach ($paquetesPromocionalesSoles as $paquete)
                                ...
                                <div class="col-md-12">
                                    <nav aria-label="Page navigation">
                                        <ul id="paginasPaquetesPomocionalesSoles"
                                            class="pagination pagination-md d-flex justify-content-center pagPaquetesPomocionalesSoles">
                                            @if ($paquetesPromocionalesSoles->onFirstPage())
                                            ...
                                            @else
                                                ...
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ $paquetesPromocionalesSoles->previousPageUrl() }}"
                                                        aria-label="Previous"><span aria-hidden="true"><i
                                                                class="feather feather-chevron-left"></i></span></a>
                                                </li>
                                            @endif

                                            @if ($paquetesPromocionalesSoles->currentPage() < 3)
                                                @for ($i = 1; $i <= 5; $i++)
                                                    @if ($i > 0 && $i <= $paquetesPromocionalesSoles->lastPage())
                                                        @if ($i == $paquetesPromocionalesSoles->currentPage())
                                                    ...
                                                    @endif
                                                @endfor
                                            @elseif($paquetesPromocionalesSoles->lastPage() > 2)
                                                @if ($paquetesPromocionalesSoles->currentPage() > $paquetesPromocionalesSoles->lastPage() - 2)
                                                    @for ($i = $$paquetesPromocionalesSoles > currentPage() - 4; $i <= $paquetesPromocionalesSoles->lastPage(); $i++)
                                                        @if ($i > 0 && $i <= $paquetesPromocionalesSoles->lastPage())
                                                            @if ($i == $paquetesPromocionalesSoles->currentPage())
                                                                <li class="page-item active">
                                                                    <a class="page-link"
                                            ...
                                            @endif
                                            @if (
                                                $paquetesPromocionalesSoles->currentPage() >= 3 &&
                                                    $paquetesPromocionalesSoles->currentPage() <= $paquetesPromocionalesSoles->lastPage() - 2)
                                                @for ($i = $paquetesPromocionalesSoles->currentPage() - 2; $i <= $paquetesPromocionalesSoles->currentPage() + 2; $i++)
                                                    @if ($i > 0 && $i <= $paquetesPromocionalesSoles->lastPage())
                                                        @if ($i == $paquetesPromocionalesSoles->currentPage())
                                                            <li class="page-item active">
                                                                <a class="page-link"
                                                ...
                                                @endfor
                                            @endif
                                            @if ($paquetesPromocionalesSoles->hasMorePages())
                                                <li class="page-item"><a class="page-link"
                                                        href="{{ $paquetesPromocionalesSoles->nextPageUrl() }}"
                                                        aria-label="Next"><span aria-hidden="true"><i
                                                                class="feather feather-chevron-right"></i></span></a>
                                                </li>
                                                <li class="page-item"><a class="page-link"
                                                        href="paquetesPromocionales?page={{ $paquetesPromocionalesSoles->lastPage() }}"
                                                        aria-label="Next"><span aria-hidden="true"><i
